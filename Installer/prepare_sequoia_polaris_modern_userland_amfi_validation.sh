...
usage() {
  cat <<'EOF'
usage: prepare_sequoia_polaris_modern_userland_amfi_validation.sh <mounted-system-volume> <efi-config-plist> [oclp-payload-root]

Prepare a single validation boot for the modern 13.5.2 Polaris userland with
the Ivy Bridge BMI hotfixes applied and Library Validation temporarily relaxed.

This wrapper:
  1. reapplies the modern Polaris userland BMI hotfix overlay
...
MODERN_SCRIPT="${SCRIPT_DIR}/apply_sequoia_polaris_modern_userland_bmi_hotfix_overlay.sh"
...
echo "Preparing Sequoia modern Polaris userland AMFI validation boot..."
echo "  target:  $TARGET"
echo "  config:  $CFG"
