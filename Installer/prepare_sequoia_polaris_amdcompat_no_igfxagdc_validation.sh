...
usage() {
  cat <<'EOF'
usage: prepare_sequoia_polaris_amdcompat_no_igfxagdc_validation.sh <mounted-system-volume> <efi-config-plist> [bronze-compat-root] [amdshared-compat-root] [modern-polaris-payload-root]

Prepare a single validation boot that restores the last usable compat Polaris
baseline and removes only kryigfxagdc from the exact amdcompat boot-args:
...
echo "Preparing Sequoia amdcompat validation without kryigfxagdc..."
echo "  target:            $TARGET"
echo "  config:            $CFG"
