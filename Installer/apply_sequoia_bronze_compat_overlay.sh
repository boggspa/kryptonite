...
usage() {
  cat <<'EOF'
usage: apply_sequoia_bronze_compat_overlay.sh <mounted-system-volume> [bronze-compat-payload-root] [modern-polaris-payload-root]

Apply a targeted Polaris Bronze Metal compatibility overlay onto an Intel-stable
Sequoia root patch:
  - restore the newest locally-available pre-BMI2 AMDMTLBronzeDriver.bundle
  - keep the rest of the Polaris userland stack on the newer 13.5.2 payload
...
attach still crashes WindowServer inside AMDMTLBronzeDriverOld.dylib on Ivy
Bridge / other non-BMI2 CPUs.
...
TS="$(date +%Y%m%d-%H%M%S)"
BACKUP_ROOT="${TARGET}/Library/Application Support/Kryptonite/RootPatchBackups/${TS}-bronze-compat-overlay"
...
info
info "Refreshing the rest of the modern Polaris userland bundle set..."
for rel in "${MODERN_ITEMS[@]}"; do
...
info
info "Bronze compatibility overlay complete."
