...
usage() {
  cat <<'EOF'
usage: apply_sequoia_polaris_modern_userland_bmi_hotfix_overlay.sh <mounted-system-volume> [modern-polaris-payload-root]

Restore a coherent modern Polaris 13.5.2 / 4.1.4 userland onto an offline
Sequoia volume, then patch the known Ivy Bridge-hostile BMI instruction sites in
the modern Radeon userland binaries so we can test the real modern stack without
the current old/new userland mix.

Defaults:
  modern-polaris-payload-root  /tmp/oclp-universal/13.5.2

This intentionally restores these modern userland items:
  - AMDMTLBronzeDriver.bundle
  - AMDShared.bundle
  - AMDRadeonVADriver2.bundle
  - AMDRadeonX4000GLDriver.bundle

Then it applies targeted binary hotfixes to:
  - AMDMTLBronzeDriverOld.dylib
  - libAMDIL902.dylib

Use this when:
  - Intel + Polaris kexts are already loading correctly
  - external scanout is alive
  - but the rendered Radeon surface path is still corrupted and kernel logs show
    AMDRadeonX4000_AMDAccelSharedUserClient outputCount count mismatch
...
TS="$(date +%Y%m%d-%H%M%S)"
BACKUP_ROOT="${TARGET}/Library/Application Support/Kryptonite/RootPatchBackups/${TS}-polaris-modern-userland-bmi-hotfix"
...
apply_binary_patches() {
  local bronze_old="${TARGET}/System/Library/Extensions/AMDMTLBronzeDriver.bundle/Contents/MacOS/AMDMTLBronzeDriverOld.dylib"
  local amdil="${TARGET}/System/Library/Extensions/AMDShared.bundle/Contents/PlugIns/libAMDIL902.dylib"

  [ -f "$bronze_old" ] || fail "missing patched target: $bronze_old"
  [ -f "$amdil" ] || fail "missing patched target: $amdil"

  /usr/bin/perl - "$bronze_old" "$amdil" <<'PERL'
use strict;
use warnings;
...
my %patches = (
  ...
  $ARGV[1] => [
    [
      0xA7A89,
      hex_to_bytes('4983fe030f92c0c4c2f0f3ce0f95c108c10fb6c1'),
      hex_to_bytes('4983fe0319c0f7d8498d4eff4c85f10f95c108c8'),
      'libAMDIL902 hotfix #1',
    ],
  ],
);

for my $path (keys %patches) {
  open my $fh, '+<', $path or die "open $path: $!\n";
  binmode $fh;

  for my $edit (@{$patches{$path}}) {
...
info
info "Restoring modern Polaris userland bundles..."
for rel in "${MODERN_ITEMS[@]}"; do
...
info
info "Applying Ivy Bridge BMI hotfixes to modern Radeon userland..."
apply_binary_patches
info "  patched AMDMTLBronzeDriverOld.dylib"
info "  patched libAMDIL902.dylib"

info
info "Re-signing modified modern userland bundles ad-hoc..."
resign_bundle_adhoc_deep "${TARGET}/System/Library/Extensions/AMDMTLBronzeDriver.bundle"
normalize_bundle_ownership "${TARGET}/System/Library/Extensions/AMDMTLBronzeDriver.bundle"
verify_bundle_signature "${TARGET}/System/Library/Extensions/AMDMTLBronzeDriver.bundle"
info "  re-signed AMDMTLBronzeDriver.bundle"
resign_bundle_adhoc_deep "${TARGET}/System/Library/Extensions/AMDShared.bundle"
normalize_bundle_ownership "${TARGET}/System/Library/Extensions/AMDShared.bundle"
verify_bundle_signature "${TARGET}/System/Library/Extensions/AMDShared.bundle"
info "  re-signed AMDShared.bundle"

rebuild_collections
create_root_snapshot

info
info "Modern Polaris userland BMI hotfix overlay complete."
