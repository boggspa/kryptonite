...
usage() {
  cat <<'EOF'
usage: prepare_sequoia_polaris_amdcompat_no_igfxagdc_kryamdsurfguard_validation.sh <mounted-system-volume> <mounted-efi-root> [built-kext] [bronze-compat-root] [amdshared-compat-root] [modern-polaris-payload-root]
...
BOOTARGS_SCRIPT="${SCRIPT_DIR}/set_efi_boot_args_amdcompat_no_igfxagdc_kryamdsurfguard.sh"
...
echo "Preparing Sequoia amdcompat validation with AMD IOSurface plane guard..."
echo "  target:            $TARGET"
...
echo
echo "Applying exact amdcompat boot-args without kryigfxagdc and with kryamdsurfguard..."
