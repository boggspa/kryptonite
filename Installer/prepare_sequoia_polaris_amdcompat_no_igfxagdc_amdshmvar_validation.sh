...
usage() {
  cat <<'EOF'
usage: prepare_sequoia_polaris_amdcompat_no_igfxagdc_amdshmvar_validation.sh <mounted-system-volume> <mounted-efi-root> [built-kext] [bronze-compat-root] [amdshared-compat-root] [modern-polaris-payload-root]
...
Prepare a single validation boot that:
  ...
  3. enables the experimental AMD shared user-client variable-output hook
...
BOOTARGS_SCRIPT="${SCRIPT_DIR}/set_efi_boot_args_amdcompat_no_igfxagdc_amdshmvar.sh"
...
echo "Preparing Sequoia amdcompat validation with AMD shared user-client output relaxer..."
echo "  target:            $TARGET"
...
echo
echo "Applying exact amdcompat boot-args without kryigfxagdc and with kryamdshmvar..."
...
echo "Rollback to the baseline no-kryigfxagdc args if needed:"
