...
usage() {
  cat <<'EOF'
usage: prepare_sequoia_polaris_amdcompat_no_igfxagdc_kryamdsurfguard_nodebug_validation.sh <mounted-system-volume> <mounted-efi-root> [built-kext] [bronze-compat-root] [amdshared-compat-root] [modern-polaris-payload-root]
...
Prepare a single validation boot that:
  ...
  4. strips debug-oriented boot flags from the working profile
...
BOOTARGS_SCRIPT="${SCRIPT_DIR}/set_efi_boot_args_amdcompat_no_igfxagdc_kryamdsurfguard_nodebug.sh"
...
echo "Preparing Sequoia amdcompat validation with AMD IOSurface plane guard and release-lean boot-args..."
echo "  target:            $TARGET"
...
echo
echo "Applying exact amdcompat boot-args without kryigfxagdc and without debug flags..."
...
echo "Rollback to the original debug-capable kryamdsurfguard args if needed:"
